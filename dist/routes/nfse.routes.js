...
const nfse_service_1 = require("../services/nfse.service");
const router = (0, express_1.Router)();
// Certificate upload setup
const certStorage = multer_1.default.diskStorage({
    destination: (_req, _file, cb) => {
        const certDir = path_1.default.join(__dirname, '../../certificates');
        if (!fs_1.default.existsSync(certDir))
            fs_1.default.mkdirSync(certDir, { recursive: true });
        cb(null, certDir);
    },
    filename: (_req, file, cb) => {
        cb(null, `certificate_${Date.now()}${path_1.default.extname(file.originalname)}`);
    }
});
...
router.get('/:id/xml', async (req, res) => {
    try {
        const record = await database_1.db.prepare('SELECT xml_rps, numero_rps FROM nfse_records WHERE id = ?').get(req.params.id);
        if (!record)
...
router.post('/emit', async (req, res) => {
    try {
        const { client_id, discriminacao, codigo_servico, valor_servicos, valor_deducoes = 0, iss_retido = false, aliquota_iss, competencia, simulate = false } = req.body;
        ...
        // Generate RPS number
        const lastNfse = await database_1.db.prepare('SELECT numero_rps FROM nfse_records ORDER BY created_at DESC LIMIT 1').get();
        const nextRps = lastNfse ? (parseInt(lastNfse.numero_rps) + 1).toString() : '1';
        const today = new Date().toISOString().split('T')[0];
        const compMonth = competencia || today;
        const nfseId = (0, uuid_1.v4)();
        // Generate XML
        const xmlRps = (0, nfse_service_1.generateRpsXml)({
            numero_rps: nextRps,
            serie_rps: '1',
            data_emissao: today,
        ...
        const signedXml = (0, nfse_service_1.signXml)(xmlRps, `RPS${nextRps}`);
        let nfseStatus = 'pending';
        let numeroNfse = '';
        let protocolo = '';
        let errorMessage = '';
        if (simulate || settings?.nfse_environment === 'simulacao') {
            const sim = (0, nfse_service_1.simulateNfseEmission)(nextRps);
            nfseStatus = 'authorized';
            numeroNfse = sim.numero;
            protocolo = sim.protocolo;
        }
        else {
            const result = await (0, nfse_service_1.sendToWebservice)(signedXml, settings?.nfse_environment || 'homologacao');
            if (result.success) {
    ...
    `).run(nfseId, numeroNfse, nextRps, '1', 'RPS', today, compMonth, client_id, discriminacao, codigo_servico, valorServicos, valorDeducoes, valorIss, aliquota, iss_retido ? true : false, valorLiquido, nfseStatus, signedXml, protocolo, errorMessage);
      ...
      `).run(txId, `NFS-e ${numeroNfse || nextRps} — ${discriminacao.substring(0, 60)}`, valorServicos, today, client_id, nfseId, `NFS-e Nº ${numeroNfse || 'Pendente'} | RPS ${nextRps}`, taxAmount);
            await database_1.db.prepare('UPDATE nfse_records SET transaction_id = ? WHERE id = ?').run(txId, nfseId);
            await database_1.db.prepare(`
...
router.post('/:id/cancel', async (req, res) => {
    try {
        await database_1.db.prepare(`UPDATE nfse_records SET status = 'cancelled', updated_at = NOW() WHERE id = ?`)
            .run(req.params.id);
...
//# sourceMappingURL=nfse.routes.js.map