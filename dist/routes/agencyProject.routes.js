...
router.post('/', async (req, res) => {
  try {
    ...
    const row = await database_1.db.prepare(`INSERT INTO agency_projects (client_name,client_code,production_start_date,deadline_days,estimated_delivery_date,total_amount,amount_paid,gateway_fee,payment_method,project_link,briefing_link,status,notes) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?) RETURNING *`).get(client_name, client_code||'', production_start_date, deadline_days||30, estimated_delivery_date||production_start_date, total_amount||0, amount_paid||0, gateway_fee||0, payment_method||'pix', project_link||'', briefing_link||'', status||'em_producao', notes||'');
...
router.put('/:id', async (req, res) => {
  try {
    const { client_name, client_code, production_start_date, deadline_days, estimated_delivery_date, total_amount, amount_paid, gateway_fee, payment_method, project_link, briefing_link, status, notes } = req.body;
    const row = await database_1.db.prepare(`UPDATE agency_projects SET client_name=COALESCE(?,client_name),client_code=COALESCE(?,client_code),production_start_date=COALESCE(?,production_start_date),deadline_days=COALESCE(?,deadline_days),estimated_delivery_date=COALESCE(?,estimated_delivery_date),total_amount=COALESCE(?,total_amount),amount_paid=COALESCE(?,amount_paid),gateway_fee=COALESCE(?,gateway_fee),payment_method=COALESCE(?,payment_method),project_link=COALESCE(?,project_link),briefing_link=COALESCE(?,briefing_link),status=COALESCE(?,status),notes=COALESCE(?,notes),updated_at=NOW() WHERE id=? RETURNING *`).get(client_name,client_code,production_start_date,deadline_days,estimated_delivery_date,total_amount,amount_paid,gateway_fee,payment_method,project_link,briefing_link,status,notes,req.params.id);
