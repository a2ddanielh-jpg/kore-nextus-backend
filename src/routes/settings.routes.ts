  ...
  try {
    ...
    await db.prepare(`
      UPDATE settings SET
        ...
        updated_at=NOW()
      WHERE id=1
    `).run(company_name, cnpj, inscricao_municipal, razao_social, endereco, municipio, uf, cep, telefone, email,
      regime_tributario ? Number(regime_tributario) : null,
      aliquota_iss ? parseFloat(aliquota_iss) : null,
      codigo_servico,
      tax_reserve_percent ? parseFloat(tax_reserve_percent) : null,
      certificate_password, nfse_environment);
    res.json(await db.prepare('SELECT * FROM settings WHERE id = 1').get());
  } catch (e: any) { res.status(500).json({ error: e.message }); }
});
